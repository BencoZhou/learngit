...
bool SingleDeviceStop(u8 index)
{
    ...
    {
        if(S1S2ControlParaGet(index)->stateByte != SWITCH_VALVE_PARA_CLOSE && S1S2ControlParaGet(index)->stateByte != SWITCH_VALVE_PARA_CLOSING)
        {                    
...
bool SingleDeviceControl(u8 index)
{
    if(S1S2ControlParaGet(index)->isSelect == TRUE)
    {
        if(S1S2ControlParaGet(index)->cDevice.place.type == DEVICE_NAME_IN_TOWERS
            ||S1S2ControlParaGet(index)->cDevice.place.type == DEVICE_NAME_MAIN_VICE)
        {
            if(S1S2ControlParaGet(index)->stateByte != SWITCH_VALVE_PARA_OPEN && S1S2ControlParaGet(index)->stateByte != SWITCH_VALVE_PARA_OPENING)
            {                    
        ...
        {
            if(S1S2ControlParaGet(index)->stateByte != TOWERSOUT_PARA_FOREWARD && S1S2ControlParaGet(index)->stateByte != TOWERSOUT_PARA_FOREWARDING)
            {                    
                DeviceSendCmd(index, SEND_TYPE_CONTROL_START);
                return TRUE;
            }
