...
extern OS_MsgBoxHandle gControlRc_Queue;
...
//            evt.Subtype = Perip_Foodline_Rc;
//            if(OS_MsgBoxSend(gControlRc_Queue, &evt, OS_NO_DELAY, FALSE) != OS_ERR_MBOX_FULL)
//            {
//            }
            ...
            switch(msg[6])
            {
                ...
                case DEVICE_CMD_FLINE_SET:
                    ...
                    {
                        if(S1S2ControlParaGet(i)->cDevice.placeNew.useID == tempDevice.placeNew.useID)
                            break;
                    }
                    S1S2ControlParaGet(i)->RcControlAlarm = msg[7];
                    if(tempDevice.place.type == DEVICE_NAME_FLINE)
                ...
                case DEVICE_CMD_FLINE_READ:
                    ...
                    if(tempDevice.place.type == DEVICE_NAME_FLINE)
                    {
                        if(FOODLINECONTROL_RC_DATA_LEN == msg[7])
                        {
                            ...
                            {
                                S1S2ControlParaGet(i)->stateByte = FoodlineControlGet()->state;
                                S1S2ControlParaGet(i)->alarmByte = FoodlineControlGet()->alarm.buf;
                                S1S2ControlParaGet(i)->isCommAlarm = FALSE;
                            }
                        }
                    }
                    else if(tempDevice.place.type == DEVICE_NAME_IN_TOWERS || tempDevice.place.type == DEVICE_NAME_MAIN_VICE)
                    {
                        if(SVALVECONTROL_RC_DATA_LEN == msg[7])
                        {
                            ...
                            if(i != S1S2DEVICE_TOTAL_NUMBER)
                            {
                                S1S2ControlParaGet(i)->stateByte = SwitchValveControlGet()->tcock;
                                S1S2ControlParaGet(i)->alarmByte = SwitchValveControlGet()->alarm.buf;
                                S1S2ControlParaGet(i)->isCommAlarm = FALSE;
                            }
                        }                        
                    }
                    else if(tempDevice.place.type == DEVICE_NAME_TOWERS_OUT)
                    {
                        if(TOWERS_OUT_RC_DATA_LEN == msg[7])
                        {
                            ...
                            if(i != S1S2DEVICE_TOTAL_NUMBER)
                            {
                                S1S2ControlParaGet(i)->stateByte = TowersOutControlGet()->motorState;
                                S1S2ControlParaGet(i)->alarmByte = TowersOutControlGet()->alarm.buf;
                                S1S2ControlParaGet(i)->isCommAlarm = FALSE;
                            }
                        } 
                    }
