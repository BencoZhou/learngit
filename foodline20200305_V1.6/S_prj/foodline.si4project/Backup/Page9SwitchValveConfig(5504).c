...
void Page9SwitchValveConfigInquiry(void)
{
    ...
    ParaSettingSendData(LocalDeviceIdGet(), gPage9DevicePara.id);
}
...
void Page9SwitchValveConfigSend(void)
{
    ...
    WirlessParaGet()->buffer[index++] = SwitchValveParaGet()->openOvertime;
    WirlessParaGet()->cmd = 0x30;
    WirlessParaGet()->len = index;
...
void Page9FlineDeviceInit(void)
{
    gPage9DevicePara.place.device = FOODLINE_STATE;
    pSwitchValvePara = SwitchValveParaGet();
    Page9FlineDeviceRefresh();
}

void Page9FlineDeviceProcess(u8 reg, u16 addr, u8 *pbuf, u8 len)
{
    u32 data;
	if(len == 4)
...
void Page9FlineDeviceRefresh(void)
{
	char str[12];
	u32 tempID;
	...
	DisplayCommNumSend(PAGE9_STATETIME          , pFoodlineDevicePara->stateTime);
	DisplayCommNumSend(PAGE9_OPEN_OVERTIME      , pFoodlineDevicePara->motorMintime);
	OSTimeDly(3);
