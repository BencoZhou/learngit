...
void Page2PathConfigRefresh(void)
{
    ...
    if(PathParameterGet()->alarm.id != 0)
    {
        DisplayCommIconSend(PAGE2_PATH_ALARM_HIDE, 0);
    ...
    else if(PathParameterGet()->alarm.next == 0xAA)
    {
        DisplayCommIconSend(PAGE2_PATH_ALARM_HIDE, 0);
        gIndex = PathParameterGet()->index;
        DisplayCommNumSend(PAGE2_PATH_ALARM_PATH, gIndex);
    	tempID = PathParameterGet()->dPara[gIndex][0].id;
    	sprintf(str, "0x%08X", tempID);
    	DisplayCommTextSend(PAGE2_PATH_ALARM_ID,	(u8*)str, sizeof(str));
    }
    else
    {
        DisplayCommIconSend(PAGE2_PATH_ALARM_HIDE, 1);
    }
